...
public class MyBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    /**
     * SheetCallback for monitoring events about bottom sheets.
     */
    ...
    public Parcelable onSaveInstanceState(CoordinatorLayout parent, V child) {
        Timber.d("onSaveInstanceState tag %s, mState %s", tag, mState);
        return new SavedState(super.onSaveInstanceState(parent, child), mState, mSlideOffset);
    }
    ...
    public void onRestoreInstanceState(CoordinatorLayout parent, V child, Parcelable state) {
        ...
        if (ss.state == STATE_DRAGGING || ss.state == STATE_SETTLING) {
            ...
            Timber.d("onRestoreInstanceState tag %s, mState STATE_COLLAPSED", tag);
        } else {
            mState = ss.state;
            Timber.d("onRestoreInstanceState tag %s, mState %s", tag, mState);
        }
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        int action = event.getActionMasked();
        if (mState == STATE_SETTLING) {//TODO mine
            return true;
        } else if (mState == STATE_HIDDEN) {//TODO mine
            return false;
        } else if (mState == STATE_DRAGGING && action == MotionEvent.ACTION_DOWN) {
            return true;
        }
        if (mViewDragHelper != null) {
...
//                setState(STATE_COLLAPSED, true);
     ...
     *              {@link #STATE_HIDDEN}.
     */
    public final void setState(final @State int state, boolean isForce) {
        if (!isForce && state == mState) {
            return;
        }
        if (mViewRef == null) {
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && mCallback != null) {
            if (top > mMaxOffset) {
                mSlideOffset = (float) (mMaxOffset - top) / (mParentHeight - mMaxOffset);
                mCallback.onSlide(bottomSheet, mSlideOffset);
            } else {
                mSlideOffset = (float) (mMaxOffset - top) / ((mMaxOffset - mMinOffset));
                mCallback.onSlide(bottomSheet, mSlideOffset);
            }
        }
    }
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcel source, ClassLoader loader) {
            ...
            slideOffset = source.readFloat();
        }

        public SavedState(Parcelable superState, @State int state, float slideOffset) {
            super(superState);
            this.state = state;
    ...
    public static <V extends View> MyBottomSheetBehavior<V> from(V view, String tag) {
        ...
        ((MyBottomSheetBehavior<V>) behavior).tag = tag;
